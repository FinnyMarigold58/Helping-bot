...
module.exports = {
    ...
    run: async(_client, message, args) => {
        ...
        if(!a0 || a0){
            embed
            .setAuthor("PWDE's Commands")
            .setDescription("$ - Permissions Requirement.")
            .addField("Guild Commands.", "("+prefix+") `guildsettings ($), setprefix ($), serverinfo`")
            .addField("Anime Commands.", "("+prefix+") `neko, nekogif, foxgirl, kemonomimi`")
            .addField("Fun Commands.", "("+prefix+") `kiss, hug, nya, slap, pat, poke, yeet, feed, rps`")
            .addField("Help Commands.", "("+prefix+") `cmds, variablereferece, translate ($), emojilist`")
            .addField("Search Commands.", "("+prefix+") `azurlane`")
            .addField("PWDE Commands.", "("+prefix+") `ping, botinfo,stand ,invite, redeem, e ($)`")
