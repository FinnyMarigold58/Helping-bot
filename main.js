...
/*
You need to change where you store the token. All sensative values like a token should only be stored in the .env file.
Put your bot token in the .env flie like token=YOURTOKENHERE 
Then replace the fs.readfile with process.env.token
Doing this prevents people from taking over your bot, and from getting you api banned by them.
Also, hosting on glitch will only keep your bot alive for about 5 min before it shuts down. You need a "keepalive" feature to ping the site, but that only works for so long.

PS (but not really): reset your token before doing the above.
I will, but what do I pu in the location?
What location
The .env
you put your token after the "token=" NO SPACES
I wil after.

Also how did you break your project so much?
You have request installed but it says its not

I was wondering why there was a error there

Ok so i seem to have fixed it
So before I remove myself can you thank me? I'm looking for that
When you press on my profile in the top left, there is an option OOOOOOOOOOOOOOOOH
Ah yeah, thanks for the help(I did)
Thank you
*/
const simkey = fs.readFileSync("./simsimi.txt", "utf8");
...
_client.on("guildMemberAdd", async (member) => {
    var memmacun = _client.guilds.get("530689610313891840").members.get(member.id);//Get Members of guild Ma CÃºn.
    var memwwo = _client.guilds.get("465795320526274561").members.get(member.id);// Get Members of guild WWO Simulation.

    if(member.guild.id == 530689610313891840){ //Guild Ma CÃºn.
        await member.guild.channels.find("id", "530689610313891843").sendMessage("ChÃ o má»«ng "+member.toString()+" Ä‘Ã£ Ä‘áº¿n vá»›i **Ma SÃ³i Discord - Ma CÃºn**!");
        await member.addRole(member.guild.roles.find("name", "Members"));
        await member.addRole(member.guild.roles.find("name", "DÃ¢n LÃ ng"));

        const embed = new Discord.RichEmbed()
        .setAuthor(member.guild.name, member.guild.iconURL)
        .addField("Ma SÃ³i Discord - Ma CÃºn", `ChÃ o má»«ng báº¡n Ä‘Ã£ Ä‘áº¿n vá»›i Server Ma CÃºn.\nVui lÃ²ng hÃ£y Ä‘á»c cÃ¡c thÃ´ng tin á»Ÿ dÆ°á»›i Ä‘Ã¢y Ä‘á»ƒ cÃ³ thá»ƒ hiá»ƒu biáº¿t vá» Server.\n- Luáº­t Server: <#530699541192638475> \n- Luáº­t Ma SÃ³i: <#587643757441187858> \n- CÃ¡ch ChÆ¡i Ma SÃ³i: <#584981323920179200> \nChÃºc báº¡n vui váº» trong Server!`)
        .setTimestamp()
        .setFooter("By Ma CÃºn Staff Team")
        .setColor("#00FF00");
        member.send(embed);
        return;
    }// End lines of Guild Ma CÃºn.

    if(member.guild.id == 580555457983152149){ //Guild Ma CÃºn - Game Server.
        ...
        if(memmacun.roles.has("534583471704899585"))//id of quáº£n trÃ² role
        { member.addRole(member.guild.roles.find("name", "Quáº£n TrÃ²")); }
        return;
    }// End lines of Guild Ma CÃºn - Game Server.
    /*
...
_client.on('guildMemberRemove', async (member) =>{
    ...
    if(member.guild.id == 530689610313891840){
        member.guild.channels.find("id", "530689610313891843").sendMessage("Táº¡m biá»‡t **"+_client.users.get(member.id).tag+"** vÃ  cáº£m Æ¡n báº¡n Ä‘Ã£ tham gia!");
    }
...
_client.on("message", async (message) => {
    ...
    if(cmd == "testinfo"){
        if(message.author.id != 454492255932252160 && message.author.id != 263472056753061889) return;
        const m = new Discord.MessageCollector(message.channel, m => m.author.id == message.author.id);
        const a = new Array;
        message.channel.send("Let us know about you!\nWhat is your name?");
        m.on("collect", async message =>{
            if(!a[0]) {a.push(message.content); message.channel.send("Name Given: "+a[0]+"\nHow old are you?");}
            else if(!a[1]) {a.push(message.content); const b = await message.channel.send("Are you want to Sumbit?\nReact below."); 
            await b.react(":yes:"); await b.react(":no:");
            const checkreact = new Discord.ReactionCollector(b, m => m.id == message.author.id)
            checkreact.on("collect", async message =>{
                    if (message.emoji == "<:yes:606770420687044618>") {
                        message.message.channel.send(`${a[0]} - ${a[1]}`)
                    }
                    else if (message.emoji == "<:no:606610883170271236>") {
                        message.message.channel.send(`Cancel Successfully.`);
                    }
                    msg.clearReactions();
                    checkreact.stop();
                    m.stop();
    ...
    if(cmd == "pingm"){
        if(message.author.id != 263472056753061889 && message.author.id != 529121242716831748) return;
        if(!args[0]) return message.channel.send("Command -pingm <Number>");
        if(args[0] > 5 || args[0] < 1) return message.channel.send("Please ping Mysterious a number of 1-5 times.");
        for (let number = 0; number < args[0]; number++) {
        message.channel.send("<@529121242716831748>")
}
    }
    ...
    if((message.guild.id == 530689610313891840 || message.guild.id == 580555457983152149) & anticmd == true){
        if(cmd == "tien"){
            if(!args[0]){
                ...
                const embed = new Discord.RichEmbed()
                .addField("TÃºi Ä‘á»“ cá»§a báº¡n!", `Xu: ${user.coins}${xu}\nHoa: ${user.roses}${hoa}\nHoa Ä‘Æ°á»£c táº·ng: ${user.plrroses}${hoa}`)
                .setColor("#00FF00");
                return message.channel.send(embed);
            }
            else{
                var getuser = func.getMember(message, args.join(" "));
                if(!getuser) return func.Loi(message, "KhÃ´ng tÃ¬m tháº¥y NgÆ°á»i ChÆ¡i.");
                var forname = ()=>{
                    if(getuser.user.id == message.author.id) return "báº¡n";
                    else return getuser.user.username
                }
                var user = GetUser(getuser.user.id);
                const embed = new Discord.RichEmbed()
                .addField(`TÃºi Ä‘á»“ cá»§a ${forname()}!`, `Xu: ${user.coins}${xu}\nHoa: ${user.roses}${hoa}\nHoa Ä‘Æ°á»£c táº·ng: ${user.plrroses}${hoa}`)
                .setColor("#00FF00");
                return message.channel.send(embed);
            }
        }
        ...
        if(cmd == "daily" || cmd == "hangngay"){
            if(message.channel.id != 660211086532018207) return;
            var user = GetUser(message.author.id);
            if(user.daily != moment().format("L")){
                ...
                const embed = new Discord.RichEmbed()
                .addField("HÃ ng NgÃ y!", ` ChÃ o má»«ng ${message.author.username} Ä‘Ã£ quay trá»Ÿ láº¡i, Báº¡n Ä‘Æ°á»£c thÆ°á»Ÿng 10${xu}`)
                .setColor("#00FF00");
                return message.channel.send(embed);
            }
            else {
                var gio = `${moment().endOf('days').fromNow()}`.replace("in ", '').replace("hours", "giá»").replace("hour", "giá»");
                var phut = `${moment().endOf('hours').fromNow()}`.replace("in ", '').replace("minutes", "phÃºt").replace("minute", "phÃºt");            
                var thoigianconlai = `${gio} ${phut}`;
                return func.Loi(message, `Báº¡n Ä‘Ã£ nháº­n thÆ°á»Ÿng trong ngÃ y hÃ´m nay rá»“i, vui lÃ²ng quay trá»Ÿ láº¡i sau ${thoigianconlai}!`)
            }
        }

        if(cmd == "themtien"){
            if(!message.guild.member(message.author).hasPermission("MANAGE_ROLES")) return func.Loi(message, "NgÆ°á»i sá»­ dá»¥ng cáº§n cÃ³ quyá»n Ä‘iá»u hÃ nh Role.")
            if(!args[0]) return func.HeThong(message, "Lá»‡nh `-themtien <NguoiChoi> <SoTien>`");
            if(!args[1]) return func.Loi(message, "Báº¡n chÆ°a nháº­p sá»‘ tiá»n muá»‘n thÃªm.");
            var getuser = func.getMember(message, args[0]);
            if(!getuser) return func.Loi(message, "KhÃ´ng tÃ¬m tháº¥y NgÆ°á»i ChÆ¡i.");
            if(isNaN(args[1])) return func.Loi(message, "Tiá»n chá»‰ cháº¥p nháº­n chá»¯ sá»‘.");

            var user = GetUser(getuser.user.id);
            user.coins = Math.floor(user.coins + parseInt(args[1]));
            SaveUser();
            func.HeThong(message, `ÄÃ£ thÃªm ${args[1]}${xu} cho ${getuser.user.username} (Giá» cÃ³ ${user.coins}${xu})`)
        }

        if(cmd == "xoatien"){
            if(!message.guild.member(message.author).hasPermission("MANAGE_ROLES")) return func.Loi(message, "NgÆ°á»i sá»­ dá»¥ng cáº§n cÃ³ quyá»n Ä‘iá»u hÃ nh Role.")
            if(!args[0]) return func.HeThong(message, "Lá»‡nh `-xoatien <NguoiChoi> <SoTien>`");
            if(!args[1]) return func.Loi(message, "Báº¡n chÆ°a nháº­p sá»‘ tiá»n muá»‘n xoÃ¡.");
            var getuser = func.getMember(message, args[0]);
            if(!getuser) return func.Loi(message, "KhÃ´ng tÃ¬m tháº¥y NgÆ°á»i ChÆ¡i.");
            if(isNaN(args[1])) return func.Loi(message, "Tiá»n chá»‰ cháº¥p nháº­n chá»¯ sá»‘.");
            ...
            user.coins = Math.floor(user.coins - parseInt(args[1]));
            SaveUser();
            func.HeThong(message, `ÄÃ£ xoÃ¡ ${args[1]}${xu} cá»§a ${getuser.user.username} (Giá» cÃ³ ${user.coins}${xu})`)
        }
    }

    if (cmd == "a"){
        message.channel.send("A láº¡i nÃ  :3");
    }

    if (cmd == "pm"){
        if(message.author.id != 454492255932252160 && message.author.id != 63472056753061889) return
    ...
    if(message.guild.id == 662181151410094081){//PWDE Support Guild ID.
        if(cmd == "verify"){
            var notverify = "662181554466062366";
            var verified = "662181550032420865";
    ...
    if(cmd == "invite"){
        ...
        var botinviteperm = "https://discordapp.com/api/oauth2/authorize?client_id=658380740865687554&permissions=2147483127&scope=bot";
        var vote = "Work in Progress";
        var serverlink = "https://discord.gg/yvrCc9q";
        const embed = new Discord.RichEmbed()
        .setAuthor("PWDE", _client.user.avatarURL)
        .setDescription(`Hello ${message.author.toString()}, Here is your choose:\n\n[Invite](${botinviteperm})\n[Vote](${vote})\n[PWDE's Server](${serverlink})\n\nYou can get Help from Staff when join PWDE's Server if you have any questions.`)
        .setColor("#FF33FF")
    ...
    if(cmd === "bo" || cmd === "botowner"){
        ...
        if(arg0.toLowerCase() == "setactivity" || arg0.toLowerCase() == "sa"){
            ...
            else if (arg0.toLowerCase() == "happynewyear" || arg0.toLowerCase() == "hny"){
            let year = fs.readFileSync("./year.txt", "utf8");
                message.channel.sendMessage(`Happy New Years: Enjoy ${year}!`);
        }
        ...
        else if (arg0.toLowerCase() == "username"){
            if (!arg1) return message.channel.sendMessage("Look like you are missing context.");
            _client.user.setUsername(args.slice(1).join(" "))
            .then(()=>message.channel.send("Changed Username to "+args.slice(1).join(" ")+""))
    ...
    if(cmd === "s"){
        ...
        if(!args[0]) return message.channel.sendMessage(`${message.author.toString()}, khÃ´ng cÃ³ lá»i gÃ¬ Ä‘á»ƒ nháº¯n thÃ¬ Ä‘á»«ng dÃ¹ng lá»‡nh =.=`).then(msg => msg.delete(5000));
    ...
    if (message.guild.id == 465795320526274561){//WWO Sim Guild
        ...
        if (cmd == "rwhost") {
            ...
            if (GlobalFunction.prototype.gamecode != null){
                ...
                GlobalFunction.prototype.hoster = user.id;
                GlobalFunction.prototype.gamecode = `RS.${count}[${args.slice(1).join(" ")}]`;
    ...
    if (message.guild.id == wwosim.id || message.guild.id == wwosimgs.id){
        ...
        if (cmd == "stopasync") {
            ...
            var getguilduser = _client.guilds.get("465795320526274561").member(message.author);
            if (!getguilduser.roles.has("606123619999023114") & !getguilduser.roles.has("606123620732895232")) return message.channel.send("Your Game Narrator or Mini Narrator roles is Missing in Werewolf Online Simulation.");
            if (!args[0]) return;
            if (args[0].toLowerCase() != "yes") return;
    
            var sendMessage = (c) => {message.guild.channels.get("606132999389708330").send(c);}
            message.guild.member(message.guild.members.get("470020987866578964")).removeRole("649925257066446848");
            sendMessage("<@470020987866578964> has been stopped in here, Don't worry, <@470020987866578964> will back in 7 secs.");
            setTimeout(()=>{
                message.guild.member(message.guild.members.get("470020987866578964")).addRole("649925257066446848");
                sendMessage("<@470020987866578964> back.");
            }, 7000);
        }
